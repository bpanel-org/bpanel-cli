...
export default async () => {
  ...
  const path = modules.indexOf('decoratePanel') > -1 ? encodeURI(name) : '';
  const metadata = Object.assign(answers, { displayName, path });

  try {
    const baseTemplate = fs.readFileSync(resolve(__dirname, '../indexTemplates/base.txt'), 'utf8');
    ...
    const newPkg =
      Object.assign(
        JSON.parse(pkgJson),
        { name, version, description, license, author, keywords: newKeywords }
      );
    await fs.writeFile(
