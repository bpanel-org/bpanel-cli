...
module.exports = (config, logger) => async (name, cmd) => {
  let bpanelConfig, CONFIG_PATH;

  try {
    const LOCAL_PLUGINS_DIR = resolve(config.prefix, 'local_plugins');
    CONFIG_PATH = resolve(config.prefix, 'config.js');
    ...
    bpanelConfig = require(CONFIG_PATH);
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      if (!fs.existsSync(resolve(LOCAL_PLUGINS_DIR, packageName))) {
        logger.warning(
